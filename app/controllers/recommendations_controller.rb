class RecommendationsController < ApplicationController
  ...
  def create
    params[:recommendation][:author] = current_user.person

    @recommendation = Recommendation.new(params[:recommendation])

    respond_to do |format|
      if @recommendation.save
        logger.debug "Next command is Postzord::Dispatcher.build(current_user, @recommendation).post"
        Postzord::Dispatcher.build(current_user, @recommendation).post
        logger.debug "Posting with Postzord::Dispatcher has completed"
